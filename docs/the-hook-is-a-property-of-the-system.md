...
Figure 1. No-wash simulations at 72 hours. The transport stress case is not spatially converged and is included to expose sensitivity to tissue geometry, not to predict penetration quantitatively.
...
Figure 2. Cumulative death commitment, membrane permeabilization, and fluorescence with reporter loss disabled or enabled. The loss scenario changes the observation process without changing the underlying killing trajectory.
...
The [Follow one simulation section](https://datarichinsightpoor.github.io/organoid-hook-model/#follow-one-simulation) makes the accounting inspectable across six time points and three dose conditions. The [worked equations](https://datarichinsightpoor.github.io/organoid-hook-model/worked-example.html) connect receptor copies, payload balance, damage, cell-state transitions, and optical weighting; the [repository](https://github.com/dataRichinsightPoor/organoid-hook-model) provides the code, parameters, trajectories, tests, and publication-record audit. Readers can examine the committed scenarios in the browser or change continuous parameters in Python.
